...
\begin{abstract}
  ...
  The memory corruption can modify the kernel data of an operating system kernel 
...
%  faces the modification of kernel data 
  ...
  %kernel data. 
  However, illegal writing of kernel data was not prevented by these existing
  security mechanisms.
  % do not prevent writing to the
    %
  Therefore, adversary can overwrite kernel data with exploiting kernel
  vulnerabilities. 
  % can be overwritten by 
  %
  % Additionally, 
  It leads that privilege escalation and the defeat of security mechanisms are
  possible.
  %
